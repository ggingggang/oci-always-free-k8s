...
- `public-gateway` (istio-system, `*.<your-domain>` listener) 준비 (`../../infra/istio/gateway.yaml`)
...
sed "s|<your-domain>|${DOMAIN}|g" httproute.yaml | kubectl apply -f -
```
...
```bash
dig +short argocd.<your-domain>
```

UI 접근: 브라우저에서 `https://argocd.<your-domain>` → admin login.
...
- 본 레포는 OSS 템플릿(placeholder + sed) — `repoURL: <your-github-user>/...` 같은 placeholder가 git에 남아있어야 함
- self-managed + `selfHeal: true` 는 git 값으로 끊임없이 복원 → cluster의 실제 URL을 placeholder로 되돌려 ArgoCD가 자기 자신을 깨뜨림
...
`public-gateway` 의 `https-wildcard` listener 가 `*.<your-domain>` 와일드카드 인증서로 TLS 종료. ArgoCD `argocd-server` 는 `--insecure` 유지하고 cluster 내부에서는 HTTP(:80) 로 listen. mesh 내부 트래픽은 Istio Ambient(ztunnel) 가 L4 mTLS 로 보호.
...
### HTTPRoute hostname placeholder

`httproute.yaml` 의 hostname 이 `argocd.<your-domain>` placeholder. sed 치환 누락 시 external-dns 가 literal `<your-domain>` 으로 DNS 시도 → Cloudflare API 4xx. apply 직후:

```bash
kubectl -n cicd get httproute argocd -o jsonpath='{.spec.hostnames}' ; echo
```
